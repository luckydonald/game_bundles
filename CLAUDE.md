...
- `src/game_collections/migrations/`: one-time migrations for the `migrate` CLI subcommand group. `tiers.py` renames legacy tier-shaped list files onto the `bundle.yml`/`tier-N.yml` convention; `bundle_variations.py` then merges each such directory (and Humble's `choice/<key>/` pick-option directories) into one flattened `<key>.yml` with `tiers`/`Game.tiers` (see `lists/README.md`).
