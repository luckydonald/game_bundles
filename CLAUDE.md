...
`src/game_collections/cli.py` is the single Typer entry point (`game-collections`). Verbs: `list`, `search NAME` (ranked matches across storefronts, `--provider`), `complete FILE` (fills draft `ids:`, `--store`/`--provider` incl. the special `isthereanydeal` provider, `--mode blank|missing|unresolved|refetch_all`), `schema`, `migrate` (one flat command, no subcommands - `--type bundle|metadata|source`, repeatable, defaults to all three; `--lists-root` for `bundle`, `--path`/`--source` for `metadata`/`source`; `--apply`/`--dry-run`/`--git`/`--git-style` shared across all three - `bundle` renames legacy tier-shaped `lists/**/*.yml` files onto `bundle.yml`/`tier-N.yml` and merges each directory into one flattened `<key>.yml` with `tiers`/`Game.tiers`, `metadata`/`source` bring archived `metadata.json`/`source.json` up to their current version envelope; dry-run by default), `scrape humblebundle`/`scrape greenmangaming` (`--url` repeatable, `--non-interactive`, `--refresh`), `scrape dailyindiegame` (`--url` repeatable, `--refresh`; opens a real browser window, see below), `scrape isthereanydeal` (`--tab live|expired|pending`, `--refresh`); `scrape humblebundle`/`scrape isthereanydeal` additionally take `--git` (autostash, run, commit the scrape's own output, restore the stash) with `--git-style manual|auto` picking the commit message wording, `eligible steam`, `sync steam` (dry-run, defaults to `--mode all --tiers highest`, `--source web|installed|collection`, `--selection-config`) / `sync steam --apply`, `apply steam` (interactive Textual picker variant of `sync`, needs the optional `tui` extra; saves `config/apply-selection.yml`), `restore steam <dir>`. All `scrape` commands log progress live ("Bundle/Offer x/y", "  Game x/y"), write each offer/bundle to disk as soon as it's ready, and resume from already-archived output by default (skipping re-fetch/re-resolution unless the archive schema changed or `--refresh`/`--no-cache` is passed). See root `README.md` for the full behavior of each verb and `lists/README.md` for the list-authoring workflow.
...
- `src/game_collections/migrations/`: migrations backing the single `migrate` CLI command. `tiers.py`/`bundle_variations.py` hold the underlying rename/merge detection logic (legacy tier-shaped list files onto `bundle.yml`/`tier-N.yml`, then each such directory - and Humble's `choice/<key>/` pick-option directories - flattened into one `<key>.yml` with `tiers`/`Game.tiers`, see `lists/README.md`); `list_versions.py` re-homes that logic under a versioned, step-by-step trajectory per bundle/choice directory (`migrate --type bundle`). `schema_versions.py` is the analogous versioned-migration engine for archived `metadata.json`/`source.json` (`migrate --type metadata|source`) - see `sources/README.md`'s "Confidence-scored dates and the version envelope" section for both.
