...
uv run game-collections sync steam --min-owned 1 --tiers all
uv run game-collections sync steam --max-missing 9999 --max-missing-pct 25 --tiers all
...
Set `STEAM_WEB_API_KEY` for ownership lookup. By default, `--source auto` tries the Web API, then the `manual-all` Steam collection, and finally the local installed-games approximation; an explicit `--collection NAME` still selects that collection directly. The most recently used account in Steam's `loginusers.vdf` is selected. `--steam-id`, `--steam-root`, `--lists-root`, and `--output-dir` provide explicit overrides.
...
`sync steam` gates eligibility on how many games in a list are owned/missing, using eight independent, optional numeric bounds: `--min-owned`, `--max-owned`, `--min-missing`, `--max-missing`, plus their percentage siblings `--min-owned-pct`, `--max-owned-pct`, `--min-missing-pct`, `--max-missing-pct`. All default to unset except `--max-missing`, which defaults to `0` - requiring every game in a list to be owned, same as before. Explicitly widen it (e.g. `--max-missing 9999`) alongside `--min-owned 1` to instead require at least one owned game with no real upper bound on what's missing. Combine bounds to find "almost complete" bundles - e.g. `--min-missing 1 --max-missing 3` surfaces lists where you're missing between one and three games. The `-pct` bounds are percentages (`0-100`) of a list's total counted games (e.g. `--max-missing-pct 25` accepts up to a quarter missing regardless of the list's size) - they're vacuously satisfied for a list with zero counted games, and combine with AND against every other bound in effect, including their own absolute-count sibling (e.g. `--max-missing 1 --max-missing-pct 25` requires both). Games without Steam IDs (an `unresolved:` marker, or a storefront game collections doesn't build URLs for) are excluded from every count by default; `--unresolved-handling`/`--unsupported-store-handling` (`hide`/`ignore`/`enforce`, default `ignore`) control this independently - `hide` makes any bundle containing that game unavailable, `ignore` leaves the bundle available and reports its name via `--log-skips`, and `enforce` folds the game into the missing count (it can never be "owned"). A `pick_quota` list (see Humble Choice below) always overrides the numeric bounds, but not `hide`. `--tiers highest` groups sibling lists by bundle directory using each list's `tier:` field (a single-tier bundle has no `tier:` field and is always included) and keeps only the numerically highest matching tier per directory. Use `--tiers all` to export every matching tier. Every bundle-writing scraper sets `tier:` itself; `game-collections migrate-tiers` (`--apply` to write, dry-run by default) brings already-generated `lists/**/*.yml` files onto this convention — a lone tier is renamed to `bundle.yml` with no `tier:` field, siblings become `tier-1.yml`, `tier-2.yml`, ... with the field set.
...
`--source none` deliberately skips ownership verification: selected lists remain subject to normal item/date, tier, and manual-selection filters, but ownership bounds and unresolved/unsupported-store ownership handling are ignored. Every listed `steam:` ID is added to its collection, including games the account may not own. Every command requires an explicit confirmation before using this mode; `--apply` still requires its normal candidate inspection and typed replacement confirmations. If `auto` cannot resolve a source, `eligible` and `sync` exit with the attempted-source diagnostics; `apply` instead opens a Textual chooser for API, collection, installed, or none.
...
The item-count/date filters, `tiers: highest`, and the min/max-missing ownership gate are all one "filtered out or not" concept, with no separate tracking of why a given bundle is excluded: a bundle failing any of them is deselected (one-way - widening a filter back doesn't re-select anything, you'd have to `enter` it again) and, by default, simply isn't in the list the tree is built from. A `show filtered (as unchecked)` checkbox adds them back to that list too, unchecked, so you can review and manually check any of them back in - but if you turn it back off, they're filtered out again, full stop; anything you'd checked among them just isn't part of the result anymore, exactly as if it'd never been checked. Bundles that don't fail any filter are always shown, checked or not. Switching `tiers` to `highest` immediately unchecks every non-highest sibling tier (and switching back to `all` rechecks them), so the picker's checkmarks preview the same "highest tier per bundle directory" grouping `--tiers` applies at plan time. `ctrl+a` selects or deselects every bundle currently passing the item-count/date filters in one keystroke (toggling: all-selected → none, anything else → all). Below the tree, a status line reads e.g. `12/40 shown` and `9/40 selected` - how many of the 40 total bundles are currently in the tree, and how many would actually end up in the saved selection.

Expanding a bundle further reveals its individual games as a third tree level; expanding a game one level further shows a menu of `Store: <provider>` entries (one per storefront it's known on, e.g. Steam/GOG/Epic) and a `Launch on Steam` entry, greyed out and inert if the game has no Steam ID. `enter` on any of these opens it via the OS's default handler — a browser for store pages, the local Steam client for `steam://rungameid/<appid>`. The picker resolves your Steam ownership up front (via the same `--source`/`--collection`/`--api-key` as `sync steam`) so it can mark each bundle with an `(owned/total)` game count and grey out individual games you don't own yet, though the submenu (store links, launch) stays reachable either way; when `--source auto` cannot resolve ownership, the picker opens a source chooser instead. Choosing none requires an explicit warning confirmation and leaves the picker unmarked. The picker exposes `--min-missing`/`--max-missing`/`--min-missing-pct`/`--max-missing-pct` as its own number-input fields, `--unresolved-handling`/`--unsupported-store-handling`/`--tiers` as dropdowns (`hide|ignore|enforce`, `highest|all`), and item-count/date bounds plus a "show filtered" toggle as picker-only fields (no CLI flag); whatever you leave them at is what's used for planning. Each of `--min-missing`/`--max-missing`/`--min-missing-pct`/`--max-missing-pct`/`--unresolved-handling`/`--unsupported-store-handling`/`--tiers` resolves per-flag as: the explicit CLI flag if passed, else the matching value from a previously saved `config/apply-selection.yml` (`--selection-config` to change the path) if one exists, else its hardcoded default (unset/`0`/unset/unset/`ignore`/`ignore`/`highest`); the item/date bounds and "show filtered" have no CLI flag at all and always come from the saved selection (or their default) alone. Clear both min/max-missing inputs to skip ownership gating entirely: every bundle you've checked in the picker (or pass via a saved `--selection-config`) is treated as eligible regardless of what you actually own, letting you deliberately sync bundles you don't have any games from yet - which is also why leaving both blank doesn't auto-hide 0-owned bundles the way a `--max-missing` bound does; it needs to show everything (just greyed) to build that manual selection in the first place. `--min-owned`/`--max-owned`/`--min-owned-pct`/`--max-owned-pct` stay CLI-only for now (no picker fields yet). Saving (`ctrl+s` or the Save button) writes your selection, along with every filter's current value, to `config/apply-selection.yml`, then offers Dry run, Apply to Steam, or Close - reopening the picker later (or right after a dry run) restores those filters the same way. A dry run returns to that action menu with the same saved selection so you can apply it afterward. `sync steam` and `eligible steam` also accept `--selection-config`, so a saved selection filters those too; a missing selection config is silently ignored (no filtering, matching prior behavior). A bundle absent from both the `selected` and `excluded` lists in the config is included by default.
...
Steam-visible collection names beginning with `🗃️ ` are reserved for Game Collections and form its managed namespace. Each sync reconciles that namespace to the current ownership-bounds/`--tiers` selection: selected collections are created or updated, while stale, superseded, no-longer-matching, and orphaned managed collections are staged for deletion. The collection used by `--source collection` is always protected. Retained collections preserve manually added games, unrelated non-prefixed collections and opaque namespace values remain untouched, and dynamic managed collections, same-name collisions, and unsafe IDs fail closed. Every deletion uses the same inspectable candidates, backups, Steam-stopped check, typed confirmation, atomic replacement, verification, rollback, and restore path as an update.
