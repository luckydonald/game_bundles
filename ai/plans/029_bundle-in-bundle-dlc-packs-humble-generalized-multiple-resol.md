...
- Manually confirm `steam://openurl/https://store.steampowered.com/dynamicstore/userdata` actually opens Steam's embedded browser to that page on the user's real machine — this can't be exercised in a sandboxed dev session (no display/Steam client here), so it needs a real check by the user before relying on it, per this repo's "verify, don't guess" practice.
- Manually run `sync steam --source auto` with the dump file present vs. absent and confirm `dynamicstore` wins when present and the loop falls through to `api` when it's missing; run once with `--refresh-dynamicstore` and once with `--no-refresh-dynamicstore` to confirm the prompt fires/never fires as expected, and confirm bare Enter at the prompt leaves the existing dump untouched.
